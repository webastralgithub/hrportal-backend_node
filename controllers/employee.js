...
exports.getEpmList = async (req, res) => {
    ...
    try {
        ...
        await Employee.find().populate({
            path: "roleType position department salary familyInfo PermanetAddress PresentAddress"
        })
            .select("-education -workExperience -Password").exec((err, data) => {
                if (err) {
                    console.log(err);
                } else {
...
exports.deleteEmployee = async (req, res) => {
    const id = req.params.id
    try {
        const findAndDeleteEmp = await Employee.findByIdAndDelete(id)
        return res.status(200).json({
            status: true,
            data: findAndDeleteEmp,
            msg: "Data deleted"
        })
    } catch (error) {
        return res.status(500).json(error)
...
exports.deleteAllEmployee = async (req, res) => {
    const tobeDeleted=req.body.ids;
   
    try {
        const deleteData = await Employee.deleteMany({ _id:tobeDeleted });
...
exports.getPersonalDetailsByEmp = async (req, res) => {
    const employeeId = req.params.employeeId
    try {
        await Employee.findOne({ _id: employeeId }).populate("roleType position department salary familyInfo").exec((err, doc) => {
            if (err) {
                console.log(err);
            } else {
                // return res.json({})
